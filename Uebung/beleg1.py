...
def trials(n, p, count):
    result = np.random.binomial(n, p, size=count) / float(n)
    #result = []
    #for i in range(0, count):
    #    result.append(np.random.binomial(n, p) / float(n))
    return result
...
for n in nValues:
    ...
    print "standard deviation :", np.std(outcomes)
    print "arithmetic mean : ", ci[0] 
    print "estimated value: ", estimatedValue / n
    #print "Confidence Interval: " + str(ci[1]) + " - " + str(ci[2])
    print "Confidence Interval: " + str(np.percentile(outcomes, 2.5)) + " - " + str(np.percentile(outcomes, 97.5))
    print "-----------------------------------------------------------"
